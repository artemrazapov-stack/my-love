...
// Кастомный курсор с лицом.
// Для работы нужны 2 картинки в папке images:
// cursor-closed.png — лицо с закрытым ртом;
// cursor-open.png — лицо с открытым ртом.
(() => {
  ...
  window.addEventListener('pointermove', (event) => {
    ...
    document.body.classList.toggle('custom-cursor-hover', Boolean(hoveredClickable));
...
// Стартовый экран: разблокировка сайта и запуск фоновой музыки.
// Файл песни положи в папку audio с названием:
// 44tune - Твое имя.mp3
(() => {
  ...
  if (!enterScreen || !enterSiteBtn) return;
